...
class Main extends PluginBase implements Listener
{
    ...
    public function spawnBoss(string $name, array $opts = []): Boss|string
    {
        $data = $this->data->get($name);
        $drops = ($opts["drops"] ?? file_get_contents(JsonProvider::$SERVER_FOLDER . "boss/drops_" . str_replace(" ", "-", strtolower($name)) . ".txt"));
        ...
        if (!$this->getServer()->getWorldManager()->loadWorld($opts["world"] ?? $data["world"])) {
            return "Failed to load world " . $opts["world"] ?? $data["world"];
        }
        $pos = new Location($opts["x"] ?? $data["x"], $opts["y"] ?? $data["y"], $opts["z"] ?? $data["z"], $this->getServer()->getWorldManager()->getWorldByName($opts["world"] ?? $data["world"]), 0, 0);
