<?php

# Namespace
namespace Tetro\EmporiumEnchants\Core;

# Custom Enchant IDs
class CustomEnchantIds
{
    const ANGELIC = 100;
    const ANTITOXIN = 101;
    const ARMORED = 102;
    const BANDAID = 103;
    const BERSERKER = 104;
    const CLARITY = 105;
    const CURSE = 106;
    const DEATHBRINGER = 107;
    const DEFLECT = 108;
    const DIMINISH = 109;
    const CYBERNETICARMORED = 110;
    const CYBERNETICENIGHTENED = 111;
    const DRUNK = 112;
    const ENDERSHIFT = 113;
    const ENDERWALKER = 114;
    const ENLIGHTED = 115;
    const ENLIGHTENED = 116;
    const ETHEREALDODGE = 117;
    const FOCUSED = 118;
    const FROZEN = 119;
    const GODLYOVERLOAD = 120;
    const HEATWAVE = 121;
    const HEAVY = 122;
    const IMPLANTS = 123;
    const METAPHYSICAL = 124;
    const MOLTEN = 125;
    const NOURISH = 126;
    const NURTURE = 127;
    const OBSCURE = 128;
    const OBSIDIANSHIELD = 129;
    const OVERLOAD = 130;
    const PAINKILLER = 131;
    const REVIVE = 132;
    # const SAVIOR = 133;
    const SHOCKWAVE = 134;
    const TANK = 135;
    const TITANTRAP = 136;
    const VALOR = 137;
    const VOODOO = 138;
    const GEARS = 139;
    const SPRINGS = 140;
    const GLOWING = 141;
    // ------- Armour ^
    const ACCURACY = 201;
    const AERIAL = 202;
    const ASSASSIN = 203;
    const BACKSTAB = 204;
    const BLESSED = 205;
    const BLIND = 206;
    const BLOCK = 207;
    const CHARGE = 208;
    const CRIPPLE = 210;
    const DECAPITATION = 211;
    const DEMISE = 212;
    const DEMONFORGED = 213;
    const DEMONICLIFESTEAL = 214;
    const DISARMOR = 215;
    const DISINTEGRATE = 216;
    const DOMINATE = 217;
    const DOUBLESTRIKE = 218;
    const EXECUTE = 219;
    const EXPERIENCEHUNTER = 220;
    const FREEZE = 221;
    const FRENZY = 222;
    const GRAVITY = 223;
    const HALLUCINATION = 224;
    const HUNTSMAN = 225;
    const ICEASPECT = 226;
    const INCENDIARY = 227;
    const INFLUX = 228;
    const INSANITY = 229;
    const INVERSION = 230;
    const LIFESTEAL = 231;
    # const LIGHTNING = 232;
    const MANIAC = 233;
    # const MOBSLAYER = 234;
    const POISON = 235;
    const RAGE = 236;
    const SILENCE = 237;
    const SOULSTEAL = 238;
    # const SPITSWEB = 239;
    const ULTRARAGE = 240;
    const WITHER = 241;
    // ------- Weapon ^

    // -------- tool
    # const SOULBOUND = 400;
    const AUTOREPAIR = 401;

    # Unknown
    const SERGEON = 505;
    const DARKNESS = 506;
    const BLOOD = 507;
    const OVERGROWTH = 508;
    const DEMONICSHARPENED = 509;
    const SHARPEN = 510;
    # pickaxe
    const ENERGY_COLLECTOR = 1001;
    const JACKPOT = 1002;
    const LUCK = 1003;
    const METEOR_HUNTER = 1004;
    const METEOR_SUMMONER = 1005;
    const MINERS_SIGHT = 1006;
    const ORE_MAGNET = 1007;
    const ORE_SURGE = 1008;
    const SHARD_DISCOVERER = 1009;
    const SHATTER = 1010;
    const SUPER_BREAKER = 1011;
    const TRANSFUSE = 1012;
    const ALCHEMY = 1013;
}