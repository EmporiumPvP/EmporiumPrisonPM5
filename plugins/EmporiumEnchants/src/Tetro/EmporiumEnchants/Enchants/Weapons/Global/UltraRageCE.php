...
# Enchantment Class
class UltraRageCE extends ReactiveEnchantment
{

    # Register Enchantment
    public string $name = "Ultra Rage";
    ...
    public function react(Player $player, Item $item, Inventory $inventory, int $slot, Event $event, int $level, int $stack): void
    {
        ...
        if (!$player->getEffects()->has(VanillaEffects::STRENGTH()) || $player->getEffects()->get(VanillaEffects::STRENGTH())->getDuration() <= 240) {
            $player->getEffects()->add(new EffectInstance(VanillaEffects::STRENGTH(), 240, 6 * $level - 1, false));
