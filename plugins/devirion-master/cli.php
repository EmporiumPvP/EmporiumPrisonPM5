...
switch(strtolower($args[1])){
	case "install":
		...
		foreach($project["libs"] as $n => $lib){
			...
			if(!isset($lib["DialogueUI"])){
				echo "[!] Library #$n does not contain DialogueUI: attribute";
			}
			...
			if($vendor === "raw"){
				...
				if(!isset($lib["DialogueUI"]) or
					count($srcParts = explode("/", trim($lib["DialogueUI"], " \t\n\r\0\x0B/"))) === 0
				){
					echo "[!] For library #$n, 'DialogueUI' attribute is missing, skipping\n";
					continue;
				}
