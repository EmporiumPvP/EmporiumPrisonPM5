...
use Emporium\Prison\tasks\ores\LapisBlockSpawnTask;
...
class LapisMineListener implements Listener {
    ...
    public function onMine(BlockBreakEvent $event) {
        ...
        # check pickaxe type
        if($item->getNamedTag()->getString("PickaxeType") != "Stone") {
            $event->cancel();
            $player->sendMessage(TF::BOLD . TF::RED . "(!) " . TF::RESET . TF::RED . "You can only use a" . TF::BOLD . TF::GREEN . " Stone Pickaxe" . TF::RESET . TF::RED . " here!");
            return;
        }
        ...
        if($pickaxeLevel == 100) {
            ...
            switch($blockId) {
                ...
                case BlockTypeIds::LAPIS_LAZULI_ORE:
                    ...
                    if($chance === 1) {
                        # spawn coal block
                        EmporiumPrison::getInstance()->getScheduler()->scheduleDelayedTask(new LapisBlockSpawnTask($block, $blockPosition), 1);
                    } else {
                    ...
                    # energy to add
                    $energy = mt_rand(30, 40);

                    # xp to add
                    $xp = 8;

                    # auto pickup
                    if($player->getInventory()->canAddItem(VanillaBlocks::LAPIS_LAZULI_ORE()->asItem())) {
                        $player->getInventory()->addItem(VanillaBlocks::LAPIS_LAZULI_ORE()->asItem());
                    } else {
                        $player->sendMessage(TF::RED . "Your inventory is full");
                        $player->getWorld()->dropItem($block->getPosition()->asVector3()->up(), VanillaBlocks::LAPIS_LAZULI_ORE()->asItem());
                    }
                    break;
                ...
                case BlockTypeIds::LAPIS_LAZULI:
                    ...
                    # energy to add
                    $energy = mt_rand(60, 80);

                    # xp to add
                    $xp = 16;

                    # auto pickup (block)
                    if($player->getInventory()->canAddItem(VanillaBlocks::LAPIS_LAZULI()->asItem())) {
                        $player->getInventory()->addItem(VanillaBlocks::LAPIS_LAZULI()->asItem());
                    } else {
                        $player->sendMessage(TF::RED . "Your inventory is full");
                        $player->getWorld()->dropItem($block->getPosition()->asVector3()->up(), VanillaBlocks::LAPIS_LAZULI()->asItem());
                    }
                    break;
