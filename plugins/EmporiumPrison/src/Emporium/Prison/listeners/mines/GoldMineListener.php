...
use Emporium\Prison\tasks\ores\GoldBlockSpawnTask;
...
class GoldMineListener implements Listener {

    private array $ores = [ItemIds::GOLD_ORE, ItemIds::GOLD_BLOCK];
    ...
    public function onMine(BlockBreakEvent $event) {
        ...
        # check pickaxe type
        if($item->getNamedTag()->getString("PickaxeType") != "Gold") {
            $event->cancel();
            $player->sendMessage(TF::BOLD . TF::RED . "(!) " . TF::RESET . TF::RED . "You can only use a" . TF::BOLD . TF::GREEN . " Gold Pickaxe" . TF::RESET . TF::RED . " here!");
            return;
        }
        ...
        if($pickaxeLevel == 100) {
            ...
            switch($blockId) {
                ...
                case BlockLegacyIds::GOLD_ORE:
                    ...
                    if($chance === 1) {
                        # spawn coal block
                        EmporiumPrison::getInstance()->getScheduler()->scheduleDelayedTask(new GoldBlockSpawnTask($block, $blockPosition), 1);
                    } else {
                    ...
                    # energy to add
                    $energy = mt_rand(50, 60);

                    # xp to add
                    $xp = 12;

                    # auto pickup
                    if($player->getInventory()->canAddItem(VanillaBlocks::GOLD_ORE()->asItem())) {
                        $player->getInventory()->addItem(VanillaBlocks::GOLD_ORE()->asItem());
                    } else {
                        $player->sendMessage(TF::RED . "Your inventory is full");
                        $player->getWorld()->dropItem($block->getPosition()->asVector3()->up(), VanillaBlocks::GOLD_ORE()->asItem());
                    }
                    break;
                ...
                case BlockLegacyIds::GOLD_BLOCK:
                    ...
                    # energy to add
                    $energy = mt_rand(100, 120);

                    # xp to add
                    $xp = 24;

                    # auto pickup (block)
                    if($player->getInventory()->canAddItem(VanillaBlocks::GOLD()->asItem())) {
                        $player->getInventory()->addItem(VanillaBlocks::GOLD()->asItem());
                    } else {
                        $player->sendMessage(TF::RED . "Your inventory is full");
                        $player->getWorld()->dropItem($block->getPosition()->asVector3()->up(), VanillaBlocks::GOLD()->asItem());
                    }
                    break;
